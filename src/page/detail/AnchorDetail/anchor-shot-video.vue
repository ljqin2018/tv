...
<template>
  <div class="anchor-shot-detail">
    ...
    <detailtop
      ...
      :initData="detailData"
      :jsonUrl="jsonUrl"
      v-on:pressDown="keepDo"
    ></detailtop>
    <div class="recommend">
      <h4 v-if="likeShow">猜你喜欢</h4>
      <ul class="recommend-content" ref="contentBox">
        <li
          v-for="(item, index) in detailData.actorList"
          :key="index"
          :id="'box-' + index"
          :class="{ active: index === itemNo }"
        >
          <lazy-image :src="item.elementImg" :placeholder="placeholder" />
...
.recommend-content li.active {
  background: url("../../../assets/detail/anchor-recommend-active.png")
    no-repeat;
  background-size: 100% 100%;
  background-size: cover;
}
...
export default {
  ...
  created () {
    if (this.$route.params.jsonUrl) {
      ...
      let url =
        this.$route.params.jsonUrl + this.actorRecParameter + this.behindParams;
      ...
      let url =
        localStorage.getItem('detailJsonUrl') +
        this.actorRecParameter +
        this.behindParams;
      this.getDetailData(url);
    }
  ...
  methods: {
    ...
    left () {
      ...
      if (this.itemNo >= 6) {
        this.$refs.contentBox.style.marginLeft =
          -(this.itemNo - 6) * 183 + 'px';
      }
      this.itemNo--;
    },
    right () {
