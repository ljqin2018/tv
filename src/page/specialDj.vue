<template>
  <div class="special">
    <div class="special_left">
      ...
      <div
        class="bottom_ico"
        :class="{ hover: itemNo == itnum || itnum <= 5 }"
      ></div>
    </div>
    <div class="special_right">
      ...
      <VideoPlayer
        v-if="detail"
        v-bind:detailData="detail"
        v-bind:currShow="currShow"
        ref="videoplayer"
        v-bind:psId="psId"
...
.top_ico {
  width: 66px;
  height: 66px;
  background: url(../../static/images/specialDj/icons_dark_next_1_-1.png)
    no-repeat;
  background-size: 100% 100%;
  position: absolute;
  left: 238px;
...
.bottom_ico {
  width: 66px;
  height: 66px;
  background: url(../../static/images/specialDj/icons_dark_next_1_.png)
    no-repeat;
  background-size: 100% 100%;
  position: absolute;
  left: 238px;
  ...
  methods: {
    ...
    KeyEnter () {
      if (this.isHover) {
        ...
        DetailRecordHelper.recordSchedule(
          this.detail.assetId,
          this.episode,
          currentTime
        );
    ...
    changeColor () {
      $('#speaialDj' + this.itemNo + ' #specialCen .list').css(
        'color',
        '#7b7b7b'
      );
      $('#speaialDj' + this.itemNo + ' #specialCen .s_list').css(
        'color',
        '#7b7b7b'
      );
    },
